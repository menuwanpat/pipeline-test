          ...
          "APP9 RTM": {
            node(label: 'slave_02') {
              withEnv(['UPD_VALUE=0', 'DB_SERVER=cmbpde1467', 'EXTENDED_SERVER=cmbpde1467']) {
                ...
                ws('D:\\ifs\\UPD_IT\\Pub') {
                  cleanWs cleanWhenAborted: false, cleanWhenFailure: true, cleanWhenNotBuilt: false, cleanWhenSuccess: false, cleanWhenUnstable: false, patterns: [[pattern: 'Logs/AutoBuilder/**', type: 'INCLUDE'], [pattern: '', type: 'INCLUDE']]
                }
              }
            }
